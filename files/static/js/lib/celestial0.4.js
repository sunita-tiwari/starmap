...
Celestial.display = function(config) {
  ...
  var proj = projections[cfg.projection],
      ...
      adapt = 1,
      rotation = getAngles(cfg.location),
      location = [-rotation[0], -rotation[1]],
      path = cfg.datapath || "";
      path = path.replace(/([^\/]$)/, "$1\/");
...
//  svg.attr("style", "cursor: default!important");//remove while using zoom
   var strkclr = (cfg.background=='#0b1a26' || cfg.background=='#000000')?'#ffffff':'#000000'
  if (circle) {
    svg.append("path").datum(circle).attr("class", "outline").attr("d", outline).style({
   "fill":cfg.background,
   'stroke' : strkclr,
   //'stroke-width': 1.5,
   });
  ...
  if (cfg.lines.graticule) {
    if (trans == "equatorial") {
      svg.append("path").datum(graticule).attr("class", "gridline").attr("d", map).style({
     "fill": "none",
     "stroke": "#ccc",
     "stroke-width": ".3px",
     "stroke-opacity": ".8",
  ...
  function clip(coords) {
    return proj.clip && d3.geo.distance(location, coords) > halfπ ? 0 : 1;
  }
  ...
  function getAngles(coords) {
    ...
    if (!coords || trans !== 'equatorial') {
      ...
      return [rot[0] - 0, rot[1] - 77.2219, rot[2]];
    }
...
Celestial.graticule = function(svg, path, trans) {
  ...
  //circles location [0º,90º] / angle 10..170º and  location [0..180º,0º] / angle 90º
...
var settings = {
  ...
  location: null,
  date: null, // Initial location coordinates in equatorial transformation only [hours, degrees], null = default locatiion
  background: "#0B1A26", // Background color or gradient
  ...
  stars: {
    ...
    size: 3.5,       // Maximum size (radius) of star circle in pixels
  ...
  constellations: {
    ...
    lines: false,   // Show constellation lines (css-class: constline)
...
/* Default parameters for each supported projection
     ...
     clip: projection clipped to 90 degrees from locatiion
*/
var projections = {
...
function d3_eventDispatch(target) {
  ...
  // DOM element that received the source event) and `argumentz` (typically, thesettings
